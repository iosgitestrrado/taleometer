...
//            let image = UIImage(data: try! Data(contentsOf: url))
    ...
    */
    open var selectedScale: CGFloat = 4 / 4
    ...
    */
    ...
    /**
     The original color of the node before animation
    */
    open var originalStrokeColor: UIColor = Defaults.color
    ...
    */
    ...
    open var originalTexture: SKTexture?
    ...
    */
    ...
    open func selectedAnimation() {
        ...
        if let selectedFontColor = selectedFontColor, let selTexture = selectedTexture {
            label.run(.colorTransition(from: originalFontColor, to: selectedFontColor, fillTexture: selTexture))
        }
        
        if let selectedColor = selectedColor, let selectedStrokeColor = selectedStrokeColor, let selTexture = selectedTexture {
          run(.group([
            scaleAction,
            .colorTransition(from: originalColor, to: selectedColor, duration: animationDuration, fromStrokeColor: originalStrokeColor, toStrokeColor: selectedStrokeColor, fillTexture: selTexture)
          ]))
        } else {
          run(scaleAction)
        }
    ...
    open func deselectedAnimation() {
        ...
        if let selectedColor = selectedColor, let oriTexture = originalTexture {
          run(.group([
            scaleAction,
            .colorTransition(from: selectedColor, to: originalColor, duration: animationDuration, fillTexture: oriTexture)
          ]))
        } else {
          run(scaleAction)
        }
        
        if let selectedFontColor = selectedFontColor, let selectedStrokeColor = selectedStrokeColor, let oriTexture = originalTexture {
            label.run(.colorTransition(from: selectedFontColor, to: originalFontColor, duration: animationDuration, fromStrokeColor: selectedStrokeColor, toStrokeColor: originalStrokeColor, fillTexture: oriTexture))
        }
